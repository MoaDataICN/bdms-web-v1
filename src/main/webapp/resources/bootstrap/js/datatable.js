...
! function(global, factory) {
    if ("function" == typeof define && define.amd) define("/tables/datatable", ["jquery", "Site"], factory);
    ...
    /*(0, _jquery2.default)(document).ready(function($) {
        ...
        function() {
            var offsetTop = 0;
            (0, _jquery2.default)(".site-navbar").length > 0 && (offsetTop = (0, _jquery2.default)(".site-navbar").eq(0).innerHeight());
        ...
        (function($) {
            var EditableTable = {
                ...
                rowCancel: function($row) {
                    var $actions, data;
                    $row.hasClass("adding") ? this.rowRemove($row) : (($actions = $row.find("td.actions")).find(".cancel-row").tooltip("hide"), $actions.get(0) && this.rowSetActionsDefault($row), data = this.datatable.row($row.get(0)).data(), this.datatable.row($row.get(0)).data(data), this.handleTooltip($row), this.datatable.draw())
                },
                ...
                rowSave: function($row) {
                    ...
                    }), ($actions = $row.find("td.actions")).find(".save-row").tooltip("hide"), $actions.get(0) && this.rowSetActionsDefault($row), this.datatable.row($row.get(0)).data(values), this.handleTooltip($row), this.datatable.draw()
                },
